(function() {
	window.H5lock = function(obj) {
		...
		this.chooseType = Number(window.localStorage.getItem('chooseType'))
				|| obj.chooseType;
	};
	...
	H5lock.prototype.drawPoint = function() { // 初始化圆心
		for (var i = 0; i < this.lastPoint.length; i++) {
			this.ctx.fillStyle = '#FFFFFF'; // #FFFFFF
			this.ctx.beginPath();
			this.ctx.arc(this.lastPoint[i].x, this.lastPoint[i].y, this.r / 2,
					0, Math.PI * 2, true);
			this.ctx.closePath();
			this.ctx.fill();
		}
	...
	H5lock.prototype.drawStatusPoint = function(type) { // 初始化状态线条
		...
		for (var i = 0; i < this.lastPoint.length; i++) {
			...
			this.ctx.beginPath();
			this.ctx.arc(this.lastPoint[i].x, this.lastPoint[i].y, this.r, 0,
					Math.PI * 2, true);
			this.ctx.closePath();
			this.ctx.stroke();
		}
	...
	H5lock.prototype.update = function(po) {// 核心变换方法在touchmove时候调用
		...
		for (var i = 0; i < this.restPoint.length; i++) {
			...
			if (Math.abs(po.x - pt.x) < this.r
					&& Math.abs(po.y - pt.y) < this.r) {
				this.drawPoint(pt.x, pt.y);
	...
	H5lock.prototype.storePass = function(psw) {// touchend 结束之后对密码和状态的处理
		if (this.pswObj.step == 1) {
			...
			if (this.checkPass(this.pswObj.spassword, psw)) {
                this.sign = true;  // 签到成功
				document.getElementById('title').innerHTML = '签到成功';
				this.drawStatusPoint('#2CFF26');
			} else {
                this.sign = false;  // 签到失败
				this.drawStatusPoint('red');
				document.getElementById('title').innerHTML = '签到失败';
			}
		}
	}
    ...
    H5lock.prototype.makeState = function() {
        document.getElementById('title').innerHTML = '签到失败！请再次签到';
    }
	...
	H5lock.prototype.bindEvent = function() {
		...
		this.canvas.addEventListener("touchstart", function(e) {
			...
			for (var i = 0; i < self.arr.length; i++) {
				if (Math.abs(po.x - self.arr[i].x) < self.r
		...
		this.canvas.addEventListener("touchend", function(e) {
			if (self.touchFlag) {
				...
				setTimeout(function() {
                    if(self.sign){  // TODO  如果签到成功，之后怎么做？？？
                        document.getElementById('canvas').style.display = 'none';
                    }else{
                        self.reset();  // 签到失败则重新绘制签到手势
                    }
				}, 300);
			}

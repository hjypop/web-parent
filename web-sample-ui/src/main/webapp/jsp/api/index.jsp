		...
		function deleteOne(code) {
			if (confirm('您确定要删除这条记录吗？')) {
				...
				if (obj.status == 'success') {
					...
					//刷新列表
					var type_ = 'post';
					var url_ = 'ajaxPageData.do?pageSize=' + parseInt($("#select-page-size").val());
					var data_ = $("#search-form").serializeArray();
					var obj = JSON.parse(ajaxs.sendAjax(type_, url_, data_));
					if (obj.status == 'success') {
						aForm.launch(url_, 'table-form', obj).init().drawForm(loadTable);
	...
	<div class="bodywrapper">
		...
		<div class="centercontent tables">
			<div class="pageheader notab">
				<h1 class="pagetitle">OpenApi管理</h1>
			</div>
						...
						<form id="search-form" class="stdform" method="post" action="">
							<div class="formwiz">
								...
								<p>
									<label>接口方法</label>
									<span class="field">
										<input type="text" name="method" id="method" value="" class="smallinput" />
									</span>
								</p>
								<p>
									<span class="field">
										<a style="text-align: right;" href="javascript:void(0)" onclick="searchApi();" class="btn btn2 btn_blue btn_search radius50">
											<span>搜索</span>
										</a>
									</span>
								</p>
